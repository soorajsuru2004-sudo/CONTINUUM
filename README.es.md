...
pytest -q                        # ~2,501 recogidos, ~2,361 pasando, ~41 saltados en un entorno mínimo (los recuentos exactos varían)
...
CONTINUUM es una librería (`src/continuum`, 124 módulos) más una suite de tests grande (161 archivos de test, ~2,501 tests). Todos los módulos añaden y reproducen un registro de eventos encadenado:
...
En tres semanas construí CONTINUUM desde un invariante, cada hecho lleva su origen. El resultado es un registro encadenado con `verify()`, un libro mayor con deduplicación por clave estable, una puerta y un gateway que bloquean efectos no reclamados, y un motor de recuperación que sella un contrato. Cinco costuras exponen el mismo registro a Claude Code, LangGraph, LangChain, OpenAI, HTTP y OpenTelemetry. Validado con muertes reales y ~2,501 tests, imprime `0 duplicados` donde la reproducción ingenua imprime `50`.
